...
A sales strategy's success and efficiency can be influenced by a variety of elements, including product type, geography, sales channel, and seasonality, to mention a few. Prioritization of sales is an important component that is frequently disregarded. Understanding which sales to prioritize in the fast-paced world of sales can dramatically optimize resource allocation and increase overall sales performance. Sales with a high priority often necessitate fast response and can result in large income or customer retention. 

Predicting the priority of sales in the large terrain of sales activities, particularly in a global setting, can be difficult. A sale's priority level can be influenced by factors such as the region of sale, the type of goods, the sales channel, and even the time of year. Businesses are expanding their sales into new markets as marketplaces become more globalized, each with its own set of problems and dynamics. Furthermore, the introduction of internet sales platforms has introduced a new degree of complexity to the sales process.

This study dives deeply into the prediction of sales prioritization. We hope to forecast whether a specific sale will have a vital priority by analyzing data from thousands of sales histories. The predictor variables include a variety of characteristics such as the sale region, item type, sales channel, and time-related parameters such as the month and year of the sale.

To rigorously test our forecasting abilities, we will build different machine learning models using various methodologies and feature selections. Our data was rigorously partitioned into training and testing sets using an 80/20 split for a robust evaluation. We will deploy our revised models on an independent evaluation dataset devoid of the target variable following model training and internal validation, assuring an unbiased assessment of our prediction prowess.
...
The preprocessing technique and sample.split() were used for the simple Logistic Regression and Random Forest models. The preprocessing consisted of encoding categorical variables (`region`, `item_type`, `sales_channel`).
...
As the first step, we built the generalized linear model based on the transformed training dataset with categorical variables transformed to dummy variables. The variables `total_profit`, `total_cost`, `total_revenue` were dropped as they are connected with each other and derived from `unis_sold` and `units_cost` variables. Since our dependent variable takes only two values (0 and 1), we used logistic regression. To do so, the function `glm()` with `family=binomial` was used. 

```{r log_model}
# Build logistic regression model
log_model <- glm(critical_priority ~ . -total_profit - total_revenue - total_cost, data = train_data, binomial(link = "logit"), control = list(maxit = 1000))
...
results <- tibble(Model = "Model #1 - Log Regression 1000 Sales", Accuracy=conf_matrix_1$overall[1], 
                  ...
                  Precision = precision(factor(log_model_pred), factor(test_data$critical_priority), "1"),
                  AIC= log_model$aic)
...
The generalized linear model was built based on the transformed training dataset with categorical variables transformed to dummy variables. The variables `total_profit`, `total_cost`, `total_revenue` were dropped as they are connected with each other and derived from `unis_sold` and `units_cost` variables. Since our dependent variable takes only two values (0 and 1), we used logistic regression. To do so, the function `glm()` with `family=binomial` was used. 

```{r log_model_large}
# Build logistic regression model
log_model_large <- glm(critical_priority ~ . -total_profit - total_revenue - total_cost, data = train_data_large, binomial(link = "logit"), control = list(maxit = 1000))
...
The `accuracy` of the model was 75.1%, higher than for the 1000 sales. Similar to the previous model, the model didn't perform well in predicting critical priority as it was shown by the precision and recall values for class 1.
...
Next, we tried  Random Forest Regression model for the small dataset. The variables `total_profit`, `total_cost`, `total_revenue` were dropped as they are connected with each other and derived from `unis_sold` and `units_cost` variables.

```{r rf_model}
# Build a Random Forest model
rf_model <- randomForest(critical_priority ~ . -total_profit - total_revenue - total_cost, data = train_data, ntree = 100, importance = TRUE)
print(rf_model)
```


The `accuracy` of the model was 71%. Comparing to th previous models, the Random forest model better predicted critical priority as it was shown by the precision and recall values for class 1.
...
results <- rbind(results, tibble(Model = "Model #5 - Random Forest 1000 Sales", Accuracy=conf_matrix_5$overall[1], 
                  ...
                  Precision = precision(factor(rf_model_pred), factor(test_data$critical_priority), "1"),
                  AIC= NA))
...
As the last model, Random Forest Regression model for the large dataset. The variables `total_profit`, `total_cost`, `total_revenue` were dropped again.

Building the model for the larger dataset took significantly more time compared to the smaller dataset

```{r rf_model_large}
# Build a Random Forest model
rf_model_large <- randomForest(critical_priority ~ . -total_profit - total_revenue - total_cost, data = train_data_large, ntree = 100, importance = TRUE, seed = 42)
print(rf_model_large)
```


The `accuracy` of the model was 75%, the same as the previous models for 50000 sales. 
...
The accuracy of both Logistic Regression and Random Forest models were marginally greater for the "50000 Sales Records" dataset than for the "1000 Sales Records" dataset. For both datasets, the performance of Logistic Regression was consistently dismal in terms of critical priorities Precision and Recall for class 1.

The model trained on the "1000 Sales Records" dataset had a greater recall for essential priorities than the model trained on the "50000 Sales Records" dataset for Random Forest. 
...
Anyway, the Random Forest Regression looked as a better choice, it could predict some of the class 1 values. There were no requirement for preprocessing tasks such as scaling and normalization of each feature.
...
2. Nwanganga, F., & Chapple, M. (2020). Practical Machine Learning in R. https://doi.org/10.1002/9781119591542
----------------
